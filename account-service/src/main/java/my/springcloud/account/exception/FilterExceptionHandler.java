...
import my.springcloud.common.logging.DevInfoType;
import my.springcloud.common.logging.LogDetails;
import my.springcloud.common.logging.LoggerAspect;
...
public class FilterExceptionHandler extends LoggerAspect {
	...
	public ResponseEntity handleMediaTypeException(HttpServletRequest req, HttpMediaTypeNotSupportedException e) {
		...
		String logString = super.getLogString(
			this.getLogDetails(ResponseCodeType.SERVER_ERROR_41001003, SvcType.SVC08.name(), SvcClassType.F09.name(),
				UNKNOWN));
		log.info(logString);
	...
	public void onAuthFail(HttpServletResponse response, ResponseCodeType responseCodeType) throws IOException {
		String logString = super.getLogString(
			this.getLogDetails(responseCodeType, SvcType.SVC08.name(), SvcClassType.F09.name(), UNKNOWN));
		log.info(logString);
