...
public abstract class LoggerAspect {
	...
	protected String getLogString(LogDetails logDetails) {
		...
		sb.append("SEQ_ID")
			...
			.append(RandomStringUtils.randomNumeric(4)); // YYYYMMDDHHmmSSsss+{순차 8자리}
		sb.append(SEPARATOR)
			.append("LOG_TIME")
			.append(KV_SEPARATOR)
			.append(logTime14); // 로그를 파일에 Write 시점 시간, YYYYMMDDHHmmSS
		sb.append(SEPARATOR).append("LOG_TYPE").append(KV_SEPARATOR).append("SVC"); // 3자리, SVC
		sb.append(SEPARATOR)
			.append("SID")
			.append(KV_SEPARATOR)
			.append(logDetails.getSid()); // "CTN : 0100xxxxxxxx(12자리), ID : alphanumeric (50바이트)"
		sb.append(SEPARATOR)
			.append("RESULT_CODE")
			.append(KV_SEPARATOR)
			.append(
				logDetails.getResultCode()); // 8자리, "1xxxxxxx : 정보, 2xxxxxxx : 성공, 3xxxxxxx : 클라이언트 에러, 4xxxxxxx : 서버에러(서비스), 5xxxxxxx : 서버에러(연동)"
		sb.append(SEPARATOR)
			.append("REQ_TIME")
			.append(KV_SEPARATOR)
			.append(logDetails.getStartDatetime()
				.format(DateTimeFormatter.ofPattern(DFP_YYYYMMDDHHMMSSSSS))); // YYYYMMDDHHmmSSsss
		sb.append(SEPARATOR)
			.append("RSP_TIME")
			.append(KV_SEPARATOR)
			.append(logDetails.getEndDatetime()
				.format(DateTimeFormatter.ofPattern(DFP_YYYYMMDDHHMMSSSSS))); // YYYYMMDDHHmmSSsss
		sb.append(SEPARATOR).append("CLIENT_IP").append(KV_SEPARATOR).append(logDetails.getClientIp()); // 클라이언트 IP
		sb.append(SEPARATOR)
			.append("DEV_INFO")
			.append(KV_SEPARATOR)
			.append(logDetails.getDevInfo()); // 5자리, 접속 단말 타입(TV, STB)
		...
		sb.append(SEPARATOR)
			.append("SVC_NAME")
			.append(KV_SEPARATOR)
			.append(logDetails.getApplicationName()); // 32자리, 각 서비스/시스템 명
		sb.append(SEPARATOR)
			.append("DEV_MODEL")
			.append(KV_SEPARATOR)
			.append(logDetails.getDevModel()); // 50자리, 단말 모델명, 셋톱박스 common 에서 추출
	...
	public abstract LogDetails getLogDetails(ResponseCodeType responseCodeType, String svcType, String svcClass,
		String subSvcClass);
