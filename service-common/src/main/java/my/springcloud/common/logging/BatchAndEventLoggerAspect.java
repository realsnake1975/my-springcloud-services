...
public class BatchAndEventLoggerAspect extends LoggerAspect {
    ...
    public void loggingSuccess(JoinPoint joinPoint, Object obj) {
        ...
        String logString = super.getLogString(this.getLogDetails(ResponseCodeType.SUCCESS, bl.svcType().name(), bl.svcClassType().name(), bl.subSvcClassType().name()));
        log.info(logString);
    }
    ...
    public void loggingFail(JoinPoint joinPoint, Exception e) {
        ...
        BatchAndEventLogger bl = this.getLoggerAnnotation(joinPoint);
        String logString = super.getLogString(this.getLogDetails(responseCodeType, bl.svcType().name(), bl.svcClassType().name(), bl.subSvcClassType().name()));
        log.info(logString);
    }
