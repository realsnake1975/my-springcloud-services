...
public class OpenFeignFormEncoder extends FormEncoder {
	...
	public OpenFeignFormEncoder(Encoder delegate) {
		super(delegate);
		MultipartFormContentProcessor processor = (MultipartFormContentProcessor)this.getContentProcessor(
	...
	public void encode(Object object, Type bodyType, RequestTemplate template) throws EncodeException {
		if (bodyType.equals(MultipartFile.class)) {
			...
			if (files != null) {
				Map<String, Object> data = Collections.singletonMap(files.length == 0 ? "" : files[0].getName(),
					object);
				super.encode(data, MAP_STRING_WILDCARD, template);
				return;
			}
